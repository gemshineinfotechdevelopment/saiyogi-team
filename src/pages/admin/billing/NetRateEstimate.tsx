...
export default function NetRateEstimate() {
  ...
  const fetchEstimates = async () => {
    try {
      const data = await apiRequest('/estimates?type=Net-Rate');
  ...
  const getNextBillNoFromServer = async () => {
    try {
      const data = await apiRequest('/estimates/next-bill-no?type=Net-Rate');
...
return () => clearTimeout(timer);
  ...
  const handleSave = async (data: any, print: boolean = false) => {
    try {
      const saved = await apiRequest(`/estimates/${data.billNo}`, {
        method: 'PUT',
        body: JSON.stringify({ ...data, type: 'Net-Rate' })
      });
  ...
  return (
    <AppLayout title="Net-Rate Estimate" noPadding hideHeader>
      <div className="flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-muted/5 pb-24">
          {activeTab === "list" ? (
            <EstimateListView type="Net-Rate" estimates={estimates} onNew={handleNew} onMultiNew={(names) => toast.info(`Multi-new disabled.`)} onEdit={handleEdit} onDelete={handleDelete} onConvert={handleConvert} />
          ) : (
            <EstimateForm
              key={activeTab}
              type="Net-Rate"
