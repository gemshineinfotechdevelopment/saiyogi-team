...
export default function RetailEstimate() {
  ...
  const fetchEstimates = async () => {
    // ...
    try {
      const data = await apiRequest('/estimates?type=Retail');
  ...
  const getNextBillNoFromServer = async () => {
    try {
      const data = await apiRequest('/estimates/next-bill-no?type=Retail');
...
return () => clearTimeout(timer);
  ...
  const handleSave = async (data: any, print: boolean = false) => {
    try {
      ...
      const saved = await apiRequest(`/estimates/${data.billNo}`, {
        method: 'PUT',
        body: JSON.stringify({ ...data, type: 'Retail' })
      });
      ...
      toast.success(`Bill ${saved.status === 'held' ? 'held' : (saved.status === 'completed' ? 'completed' : 'updated')} successfully`);
    } catch (err) {
  ...
  return (
    <AppLayout title="Retail Estimate" noPadding hideHeader>
