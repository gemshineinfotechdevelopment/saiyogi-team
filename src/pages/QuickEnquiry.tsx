...
import { Plus, Minus, ShoppingCart, Sparkles, ShoppingBag, X } from "lucide-react";
...
const QuickEnquiry = () => {
  ...
  }, [items, settings.discountPercent]);
  ...
  const groupedProducts = useMemo(() => {
    ...
    products.forEach((p) => {
      ...
      const catName = typeof cat === 'object' && cat !== null ? cat.name : (typeof cat === 'string' ? cat : null);

      if (catId && catMap[catId]) {
        catMap[catId].items.push(p);
      } else if (catName) {
        const foundKey = Object.keys(catMap).find(k => catMap[k].categoryName === catName.toUpperCase());
        if (foundKey) {
          catMap[foundKey].items.push(p);
        } else {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white relative font-sans">
      ...
      <main className="flex-1 w-full py-8 px-4 md:px-8 max-w-7xl mx-auto">
        {/* Beautiful Top Banner */}
        <div className="bg-gradient-to-br from-[#A80000] via-[#5c0a0b] to-[#1A1A1A] text-center py-10 px-6 rounded-2xl mb-8 relative overflow-hidden border border-[#F4C542]/20 shadow-xl">
          ...
          <h1 className="text-3xl md:text-5xl font-black text-white uppercase tracking-wider mb-3 font-display">Quick Enquiry List</h1>
        ...
        <div className="flex justify-end mb-6">
          <div className="inline-flex items-center gap-3 bg-[#4A0000] text-white px-5 py-2.5 rounded-full shadow-lg border border-red-900">
            <div className="relative flex items-center justify-center">
              <ShoppingBag className="w-5 h-5 text-amber-400" />
              <span className="absolute -top-1 -right-1.5 bg-green-600 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-extrabold">
                {totalItems}
              </span>
            </div>
        ...
        <div className="w-full bg-white shadow-lg border border-gray-200 rounded-xl overflow-hidden mb-8">
          {/* Table Header */}
          <div className="hidden md:grid grid-cols-12 gap-4 pb-3 pt-4 px-6 text-[10px] font-extrabold text-gray-500 uppercase tracking-widest bg-gray-50 border-b border-gray-200">
                      ...
                      <div key={pId} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-6 hover:bg-gray-50/50 transition-colors">
                        ...
                        <div className="col-span-1 md:col-span-1 flex justify-end md:justify-end items-center">
                          <button 
                            ...
                            disabled={stock <= 0}
                            className="w-full bg-[#A80000] text-white hover:bg-[#F4C542] hover:text-[#1A1A1A] font-extrabold text-[11px] py-2.5 rounded-xl transition-all duration-300 shadow-md uppercase tracking-wider flex items-center justify-center gap-1.5 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                          >
